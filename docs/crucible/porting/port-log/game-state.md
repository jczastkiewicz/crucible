...
`DealDamage` and `GainLife` today (alongside casting a permanent and an Aura, M5's own two) — `ResolveStack` reports
`ErrUnimplemented` for the other 200. `TestCastSpellFiresETBTrigger`, `TestDestroyLethalToughnessFiresDiesTrigger`,
