...
**In progress.** 164 of the corpus's 203 script-driven `Effect` APIs resolve (`Draw`, `DealDamage`, `GainLife`, `Pump`,
...
`CopySpellAbility`) — see `docs/crucible/porting/port-log/game-state.md` for the per-API landing notes; items 30-32
below stay in their original plan-authoring voice (forward-looking, not yet rewritten as a per-item retrospective the
way M0-M5 are).
