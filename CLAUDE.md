...
M6 in progress alongside it: 22 of the corpus's 203 script-driven `Effect` APIs resolve rather than reporting
...
`SetLife`, `Shuffle`, `ExchangeLife` — each with `UnlessCost$` and `SubAbility$` chaining wired through
`Registry.Resolve` (`effect.go`); `Destroy`/`Tap`/`Untap` are the first to read a chosen target (`Ability.Targets`)
directly rather than only through `Defined$` (`targetedOrDefinedCards`, `defined.go`), and `Mill`/`Shuffle`/`SetLife`/
`ExchangeLife` do the same for a player target (`targetedOrDefinedPlayers`). `ChangeZone` (6,616 real corpus lines) is
the single largest remaining gap; real instant/sorcery casting, `DB$ Effect`/`Repeat`/`GenericChoice`/`DelayedTrigger`,
and token creation are the next largest.
