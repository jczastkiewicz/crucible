...
## Layer 7, Layer 4, Layer 5, Layer 6 and Layer 8: `PT`/`TypeMod`/`ColorMod`/`KeywordMod`/`RulesMod`, `Card.Power`/`Toughness`/`Type`/`Colors`/`HasKeyword`, and the first real Continuous callers
...
## Trigger firing: entering, dying, attacking, blocking, dealing damage, being discarded, becoming tapped, tapping for mana, casting a spell, the beginning of a step or phase, and watching another permanent
...
`TestDeclareCombatAttackersDoesNotFireTapsForManaTrigger` (trigger_test.go) prove nine of the ten modes against
synthetic Elvish-Visionary-, Rotting-Regisaur-, Blood-Artist-, Impact-Tremors- and attacking/blocking/damage-dealing/
discarded/tapping/casting-creature-shaped cards, each compiled through the real pipeline; the tenth, `Phase`, has its
own test list right after its own descriptive paragraph, above.
...
| Layers 1-3 in full (copy -- not even part of `StaticAbilityContinuous.java`'s own switch in Forge itself, a separate resolution-time mechanism; control, `GainControl$`, 42 real lines, tractable in the same "latest Timestamp wins" sense Layer 8 was but blocked on `Card.Controller` being a plain field this port mutates nowhere today, a bigger first-of-its-kind change than Layer 8's own player-facing `RulesMod` was; text, `GainTextOf$`, 1 real line), plus Layer 8's own remainder (`MayLookAt$`/`MayPlay$`, `AddHiddenKeyword$`, vote/villainous-choice params -- the Layer 7/4/5/6/8 section's own `RulesMod` paragraph, above, has the exact reasons and counts) and the rest of Layers 4/5/6 past a literal token list and Layer 7a's own SVar shapes outside the Valid family (`xPaid`, `CardCounters`, `Devotion`, ... -- the same section's own `resolveAmount` paragraph) -- `applyContinuousPT`/`applyContinuousType`/`applyContinuousColor`/`applyContinuousKeyword`/`applyContinuousRules` (continuous.go) resolve Layer 7b/7c's own plain-integer AND now Count$Valid-SVar-driven lines, Layer 7a's own Count$Valid-SVar-driven `CharacteristicDefining$` lines (`applyOneCharacteristicDefiningPT`), 201 of 284 real `AddType$`/`RemoveType$` lines, 54 of 61 real `AddColor$`/`SetColor$` lines, 1,556 of 1,857 real `AddKeyword$` lines and 75 of 78 real `SetMaxHandSize$`/`RaiseMaxHandSize$`/`AdjustLandPlays$` lines, all `Affected$`-matched; the rest all need the rest of the same general engine, mostly for keys a full `AbilityUtils.calculateAmount` port, a `*cardtype.Registry` this port does not inject into the engine, or a mechanism this port has never needed before (a cast-time zone permission, a controller-change) would resolve, not a new layer number to add                                                  | M5-M6 |
