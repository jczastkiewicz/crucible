...
| `covergate` — TEST-12 coverage floor                     | Yes                                          |
...
Checks whose tooling does not exist yet — L4-L6 — are enforced by the reviewer until it does. A gate listed here is
binding either way; only the mechanism differs.
