...
for someone else's zone change — named in the file's own doc comment as a gap, not an oversight, since it needs walking
every OTHER permanent's own triggers against the event, not this card's. `checkOtherETBTriggers` closes that gap for
entering: every permanent already on the battlefield, other than the one that just entered, gets its own `Triggers`
walked against it too (`ValidCard` matched with the watcher as source and the watcher's own controller — Impact Tremors'
`Mode$ ChangesZone | Destination$ Battlefield | ValidCard$ Creature.YouCtrl` fires off any other creature its
controller's own control enters). `TriggerZones$ Battlefield`, present on most corpus lines shaped this way, needs no
separate check: only a card this loop already found on the battlefield is walked, so a watcher not there is never
considered. The identical gap for dying is not closed the same way: a dies-shaped watcher trigger almost always carries
that same `TriggerZones$ Battlefield`, but the dying card itself is already gone from the battlefield by the time
`checkDiesTriggers` runs, so `checkOtherETBTriggers`'s "just walk the battlefield" shortcut does not transfer — a
permanent watching some OTHER creature die remains a gap.
...
Resolving it is a different question entirely: a trigger's own `Execute$` sub-ability is always one of the 203
corpus-frequency effects M6 owns (`Draw`, `Token`, `GainLife`, `DealDamage`, ...), structurally disjoint from the three
...
casting itself. `TestCastSpellFiresETBTrigger`, `TestDestroyLethalToughnessFiresDiesTrigger` and
`TestCastSpellFiresOtherPermanentsWatchingTrigger` (trigger_test.go) prove the three paths against synthetic
Elvish-Visionary-, Rotting-Regisaur- and Impact-Tremors-shaped cards, each compiled through the real pipeline: the
permanent still reaches its own zone, then the pushed ability (`Draw`, `Draw`, `DealDamage`) surfaces the gap by name.

Corpus-frequency: 5,688 cards carry the ETB shape (`Destination$ Battlefield`); 1,341 carry the dies shape
(`Origin$ Battlefield` + `Destination$ Graveyard`) — both now detected, neither yet resolved.
...
`Flying` keyword at `CardState`-build time, the identical mechanism `Fear` (`:3906-3909`, 40 cards) and `Horsemanship`
(`:3932-3935`, 29 cards) use. `cantBlockByKeywords` (`staticability.go`) reproduces that synthesis for those three
keywords, keyed off `Card.HasKeyword` rather than a literal compiled `S:` line — nothing in `internal/carddb/compile`
performs Java's own keyword-expansion step, so this port's own version of it has to live here instead, one call site
rather than a compile-time rewrite. `Menace` (408 cards) is not among them: Forge itself does not run Menace through
this engine either — `getMinMaxBlocker` hardcodes `attacker.hasKeyword(Keyword.MENACE)` directly, a
minimum-blocker-_count_ rule a per-pair `CantBlockBy` check cannot express — so it stays a gap, not a cut corner,
needing a different hook (validating a `Block` group's size per attacker, not one pair) than this slice built.

`Intimidate` (23 cards) is deliberately not synthesized either, for a sharper reason than "not built yet": its own
`ValidBlocker$ Creature.nonArtifact+!SharesColorWith` needs a `SharesColorWith` property `valid.go` does not evaluate,
and the generic `non<Type>` fallthrough it would otherwise reach reads "SharesColorWith" as a nonexistent type
(`false`), which the leading `!` then negates to `true` — an actively wrong "matches everything," not an absent
property. Landwalk (133 cards, `K:Landwalk:<Type>:...`) has no `ValidBlocker` at all, only
`ValidDefender$ Player.controls<Type>` — `Matches` evaluates a `*Card`, not a `*Player`, so nothing here can check it.
...
that specific attacker — 1 real corpus card); `ValidAttackerRelative`/`ValidBlockerRelative` (1 card); `ValidDefender`
(0 real `S:CantBlockBy` lines use it); the Landwalk ignore-check (moot, since Landwalk itself is not synthesized here).
...
| Trigger firing (CR 603) beyond a permanent's own "enters"/"dies" modes and watching another permanent enter — `checkETBTriggers`/`checkOtherETBTriggers`/`checkDiesTriggers` (trigger.go) cover those three; every other mode (`Attacks`, `Tapped`, a spell being cast, ...) and a permanent watching some OTHER permanent die both remain gaps (a dies-shaped watcher trigger needs the dying card's state as it was a moment ago, not a battlefield walk `checkOtherETBTriggers` can just do), and resolving what fires (not just detecting it) is M6's 203 corpus-frequency effects, not this                                                      | M5-M6 |
...
| Block legality's remaining `CantBlockBy` gaps — Menace (a different, hardcoded-in-Forge-too minimum-blocker-_count_ mechanism, `getMinMaxBlocker`, not `CantBlockBy` at all), Intimidate (`SharesColorWith` property `valid.go` does not evaluate), Landwalk (`ValidDefender$ Player.controls<Type>` needs a `*Player` match `Matches` cannot make), Protection (needs `Protection.java`'s own valid-string builder) and Skulk (`ValidBlocker$ Creature.powerGTX` needs an SVar-driven `X`) — `## Block legality: CantBlockBy` (above) has the full account; flying/reach, Fear, Horsemanship and every literal `S:Mode$ CantBlockBy` line are ported | M5-M6 |
