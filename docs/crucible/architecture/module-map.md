...
- **Status:** Active
- **Describes:** state as of 2026-09-13
...
| [`internal/carddb/compile`](../../../crucible/internal/carddb/compile) | Ability lines to a tree: record type, API, params, and `SubAbility$` chains resolved into direct references                                                                                                               | `forge.game.ability.AbilityFactory` (`getAbility`, `getSubAbility`)                                                                                                                    | [`ability-factory.md`](../porting/port-log/ability-factory.md)                                                   |
| [`internal/carddb/vocab`](../../../crucible/internal/carddb/vocab)     | Measures the script vocabulary: every param key, API, mode, count head, cost part, valid token and keyword head the corpus uses                                                                                           | `AbilityFactory.getMapParams`, `AbilityUtils`, `CardProperty`, the `cost/` package                                                                                                     | — new code; measures the Java grammars rather than porting one                                                   |
...
| [`internal/cost`](../../../crucible/internal/cost)                     | Cost strings: what a player pays to activate an ability or cast a spell                                                                                                                                                   | `forge.game.cost.Cost` (its constructor and `parseCostPart`)                                                                                                                           | [`cost-strings.md`](../porting/port-log/cost-strings.md)                                                         |
| [`internal/expr`](../../../crucible/internal/expr)                     | Amount expressions: the numbers an ability computes rather than states                                                                                                                                                    | `forge.game.ability.AbilityUtils` (`calculateAmount`, `doXMath`)                                                                                                                       | [`count-expressions.md`](../porting/port-log/count-expressions.md)                                               |
| [`internal/engine`](../../../crucible/internal/engine)                 | The rules core: the arena every handle indexes into, cards and their parts, players, zones, effect dispatch, the controller interface, state-based actions, turn structure, mulligans, the stack, valid-string evaluation | `forge.game.Game`, `Card`, `Player`, `zone.Zone`, `zone.ZoneType`, `zone.MagicStack`, `player.PlayerController`, `GameAction`, `phase.PhaseHandler`, `mulligan.*`, `card.CardProperty` | [`game-state.md`](../porting/port-log/game-state.md), [`valid-strings.md`](../porting/port-log/valid-strings.md) |
| [`internal/fixture`](../../../crucible/internal/fixture)               | Forge's `GameState` text format: parses it, builds a `*engine.Game` from it, writes one back, and runs a scenario's `actions.log` against it                                                                              | `forge.game.GameState` (`parse`, `processCardsForZone`, `applyGameOnThread`, `toString`)                                                                                               | [`game-state-fixture.md`](../porting/port-log/game-state-fixture.md)                                             |
| [`internal/valid`](../../../crucible/internal/valid)                   | Valid strings: the predicates deciding what an ability may target, affect or count                                                                                                                                        | `forge.game.card.CardProperty`, `forge.util.Expressions`                                                                                                                               | [`valid-strings.md`](../porting/port-log/valid-strings.md)                                                       |
| [`internal/keyword`](../../../crucible/internal/keyword)               | `K:` lines: the head, its positional details, and the enum entry saying how they are shaped                                                                                                                               | `forge.game.keyword.Keyword` and its `KeywordInstance` subclasses                                                                                                                      | [`keywords.md`](../porting/port-log/keywords.md)                                                                 |
...
| [`tools/vocabscan`](../../../crucible/tools/vocabscan)                 | Prints that vocabulary, and generates `porting/parity-matrix.md` from it and the effect registry                                                                                                                          | — new code                                                                                                                                                                             | —                                                                                                                |
...
`internal/carddb` turns a card script into a value, drawing on `internal/mana` and `internal/cardtype`.
`internal/engine` is the one-way arrow ADR-0003 describes: it imports `internal/carddb/compile` for the shared,
immutable card database every game runs against, and nothing imports back into it. `internal/fixture` sits above both:
it builds an `*engine.Game` from a fixture's text, so it needs `compile.DB` the same way the engine's own callers will
once something loads a real corpus rather than a test's three named cards.
...
flowchart LR
  ...
  db --> compile["internal/carddb/compile"]
  compile --> engine["internal/engine"]
  engine --> fixture["internal/fixture"]
  compile --> fixture
```
