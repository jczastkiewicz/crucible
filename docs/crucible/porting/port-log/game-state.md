...
89 of the corpus's own 118 real `Mode$ BecomesTarget` lines resolve now. Not resolved, each failing loudly by name
rather than matching unconditionally (PORT-8/GO-7): `OptionalDecider$` (12) -- an interactive "may" confirm this port's
own `PlayerController` has no hook for, the identical gap `Discard`'s own `Optional$` already documents; `Valiant$` (10)
-- `Card.isValiant`'s own separate per-activator "have you not targeted this before" set
