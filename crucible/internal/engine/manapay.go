...
// resolve each two-colour hybrid, monocoloured hybrid, colourless hybrid,
// single-colour Phyrexian and hybrid Phyrexian shard (CR 601.2h, CR 118.4),
// then which mana type covers each unit of the cost's generic amount (CR
// 106.6), before handing the result to [Pool.Pay]. Reports whether it
// succeeded; the pool is unchanged if it did not, the same guarantee Pay
// itself makes -- and life is deducted only after Pay reports success, so a
// Phyrexian shard resolved to life never costs life on a payment that fails
// for an unrelated shard.
//
// A shard offering anything else -- {X} or snow -- is not resolved here: it
// passes through unchanged, and Pay's own "unresolvable shard" branch fails
// the whole payment for it, exactly as it already does today. Each is a
// real decision with no PlayerController method built for it yet --
// ChooseHybridManaColor, ChoosePayMonocoloredHybrid, ChoosePayColorlessHybrid,
// ChoosePayPhyrexian, ChoosePayHybridPhyrexian and ChoosePayGeneric are the
// first six, not the last (game-state.md's "Mana pool and payment"
// section).
