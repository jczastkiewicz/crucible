...
// Defined$ You | NumCards$ 3 | SubAbility$ DBSubgame, chaining into DB$
// Subgame (Shahrazad, an API this port has not built yet). Draw's own
// three cards must
...
func TestSubAbilityChainUnimplementedAPIErrors(t *testing.T) {
	...
	def := etbSubAbilityTriggerDefParams(t, "Test Riverwise Augur",
		"DB$ Draw | Defined$ You | NumCards$ 3 | SubAbility$ DBSubgame",
		map[string]string{"DBSubgame": "DB$ Subgame"})

	c := engine.NewScriptedController()
	err := castETBSubAbility(t, g, p, def, c)
	if err == nil {
		t.Fatal("ResolveStack: got nil error, want one naming Subgame")
	}
	if !errors.Is(err, engine.ErrUnimplemented) {
		t.Errorf("ResolveStack error = %q, want it to wrap ErrUnimplemented", err.Error())
	}
	if !strings.Contains(err.Error(), "Subgame") {
		t.Errorf("ResolveStack error = %q, want it to name Subgame", err.Error())
	}
	if len(g.Zone(engine.Hand, p).Cards()) != 3 {
		t.Errorf("p's hand size = %d, want 3 -- Draw's own body must already have run before the chain hit Subgame",
