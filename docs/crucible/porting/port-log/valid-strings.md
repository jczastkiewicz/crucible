...
- **Status:** Parsing done — M3 slice C. Evaluation started in `internal/engine` (M5): `Matches` covers the
  ...
  Aura leg filtered to the modified card's own controller); controller/owner relative to `sourceController` (`YouCtrl`,
  ...
  type/supertype/subtype fallthrough every property chain shares. The other ~886 property names are M5-M6, corpus
