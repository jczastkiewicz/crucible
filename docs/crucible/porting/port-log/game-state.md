...
Every real `Event$ GainLife | ReplaceWith$` line (20) needs either `DB$ ReplaceEffect` (a dedicated API most of the real
"double your life gain"/"gain 1 extra life" lines target, 15) or "the amount of life that would have been gained" as a
runtime value (`ReplaceCount$LifeGained`, the other 5, nefarious_lich.txt's/rain_of_gore.txt's/
tainted_remedy.txt's/lich.txt's/plague_drone.txt's own real lines among them) -- neither shape this dispatch's own
"already-built leaf ability, nothing else left to resolve" contract covers, so `GainLife` is untouched by this chunk;
both stay exactly the gap `gainLifePrevented`'s own doc comment (above) already names.
...
| CR 616's own general "more than one replacement effect could apply, the affected player chooses" ordering procedure — needs a `PlayerController` hook this port does not have; moot for four resolved shapes' own idempotent outcome (`Tapped = true`, blocked, prevented) but real for `drawReplaced`'s own non-idempotent one (no real corpus deck combines two Draw-replacing permanents today, so not yet observable). Most `Event$` values past `Moved`/`Untap`/`DamageDone`/`Draw`/`GainLife` (`Counter`, `AddCounter`, `CreateToken`, `BeginPhase`, `GameLoss`, `ProduceMana`, ...), `Draw`'s own remaining 33 real `ReplaceWith$` lines and every real `GainLife` `ReplaceWith$` line (20), stay unresolved too (`## Replacement effects`, `## ReplacementEffect.requirementsCheck lands`, above)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | M5-M6 |
